...
We have the option of writing out the code in the file inside the \verb|lstlisting| environment or import from file with \verb|\lstinputlisting{}|. Try it for yourself! Remember that an environment is always \verb|\begin{environment-name}|\dots\verb|\end{environment-name}|.
\begin{lstlisting}
    \lstinputlisting[language=Matlab, caption={caption-here}, label={label-here}]{code/code.m}
\end{lstlisting}
