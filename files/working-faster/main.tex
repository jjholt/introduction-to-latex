...
\begin{enumerate}
    ...
    \item To create \( \frac{m}{n}  \), \keys{@/ + \tab} for the fraction, then \keys{m + \tab + n}
    \item For the next maths environment, we use \keys{bsal+\tab} --- creating an \texttt{align*} environment
    \item To create \( 2^{m/n} \) quickly, we type \keys{2 + ** + \tab + m/n + \tab}.
    \item Finally, to write \( {2^{m/n}}^n \), we need to think ahead about the position of \( ^n \). \keys{\{ + 2 + ** + \tab + m/n } will produce \verb|{2^{m/n}}|. Move to the end of the line with \keys{\ctrl+\arrowkeyright} (Mac: \keys{\cmd+\arrowkeyright}), then you can finish writing \verb|{2^{m/n}}^n|.
\end{enumerate}
